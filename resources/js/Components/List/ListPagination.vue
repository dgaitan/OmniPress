      	...
      	<div class="container px-4 mx-auto">
        	<div class="flex flex-wrap -mx-4 items-center justify-between">
	          	...
	          	<div class="w-full lg:w-auto px-4 flex items-center justify-center">
	          		<p class="text-md text-gray-400 mr-3">Page {{ params.currentPage }} of {{ totalPages }}</p>
		            <Link v-if="params.prevUrl" :href="params.prevUrl" class="inline-flex mr-3 items-center justify-center w-8 h-8 text-xs text-gray-500 border border-gray-300 bg-white hover:bg-indigo-50 rounded">
	            	...
	            	<input v-model="page" type="text" v-on:keyup.enter="changePage()" class="inline-flex mr-3 items-center justify-center w-8 h-8 text-xs text-gray-500 border border-gray-300 bg-white hover:bg-indigo-50 rounded w-20 text-center" />
	            	<Link v-if="params.nextUrl" :href="params.nextUrl" class="inline-flex items-center justify-center w-8 h-8 text-xs text-gray-500 border border-gray-300 bg-white hover:bg-indigo-50 rounded">
	              		<svg width="6" height="8" viewBox="0 0 6 8" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script>
	...
	export default defineComponent({
		...
		created() {
			this.page = this.params.currentPage
			this.perPage = this.params.perPage
		},

		computed: {
			totalPages() {
				return Math.ceil(this.params.total / this.params.perPage)
			}
		},
